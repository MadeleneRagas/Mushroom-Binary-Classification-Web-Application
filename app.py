...
def main():
    ...
    def plot_metrics(model, metrics_list):
        if 'Confusion Matrix' in metrics_list:
            ...
            # Changed from plot_confusion_matrix() to ConfusionMatrixDisplay.from_estimator()
            cm = ConfusionMatrixDisplay.from_estimator(model, x_test, y_test)
            st.pyplot(cm.figure_)  # Pass figure to Streamlit

        if 'ROC Curve' in metrics_list:
            st.subheader("ROC Curve")
            # Changed from plot_roc_curve() to RocCurveDisplay.from_estimator()
            roc = RocCurveDisplay.from_estimator(model, x_test, y_test)
            st.pyplot(roc.figure_)

        if 'Precision-Recall Curve' in metrics_list:
            st.subheader("Precision-Recall Curve")
            # Changed from plot_precision_recall_curve() to PrecisionRecallDisplay.from_estimator()
            pr = PrecisionRecallDisplay.from_estimator(model, x_test, y_test)
            st.pyplot(pr.figure_)

    # --- Sidebar: Choose classifier ---
    ...
    # --- Logistic Regression Classifier ---
        ...
        if st.sidebar.button("Classify LR", key='classify_lr'):
    ...
    # --- Random Forest Classifier ---
        ...
        if st.sidebar.button("Classify RF", key='classify_rf'):
    ...
    if st.sidebar.checkbox("Show raw data", False):
        ...
        st.markdown(
            ...
            "corresponding to 23 species of gilled mushrooms. Each species is identified as definitely edible, definitely poisonous, "
            "or of unknown edibility and not recommended. The latter class was combined with the poisonous one."
        )
